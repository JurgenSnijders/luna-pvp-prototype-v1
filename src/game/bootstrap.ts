...
function init(app: GameApp): void {
  ...
  app.loop = new Loop({
    ...
    onRender(alpha, frameDt) {
      ...
      const streakTarget = getEffectiveCrtSettings().grade.streakTarget;
      const cameraView = {
        ...
        streakBodies: streakTargetNeedsExtract(streakTarget)
          ? app.renderer.collectStreakBodies(
              app.world,
              alpha,
              streakTargetFiltersEntities(streakTarget),
            )
          : undefined,
      };
      ...
      AudioEngine.getInstance().update(performance.now());
