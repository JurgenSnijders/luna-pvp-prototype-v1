...
trajectory: { type: LINEAR|RETURN_TO_SOURCE|ORBIT_ANCHOR|HOMING_SLERP|DISCONTINUOUS_BLINK|BALLISTIC_ARC|DRAWN_PATH, speed, maxRange, piercing?, turnAccel?, orbitRadius?, orbitSpeed?, blinkDistance?, lobApex?, spawnAltitude?, fallSpeed?, bounces?, bounceRestitution?, clearanceHeight?, detonateAtZ?, gravityScale?, groundFriction?, pathPoints?, pathSpace?, pathLoop?, motion? }
  motion?: { wobble?:{ amplitudeDeg:0-45, frequencyHz:0.5-8, decay?:0-4 }, jitter?:{ magnitude:0-80 }, drift?:{ lateralAccel:0-800 }, spiral?:{ radius:0-80, frequencyHz:0.5-8 }, speedCurve?:{ startScale:0.15-1, rampMs:80-1500 } } — optional post-pass modifiers; combine sparingly (do not pair orbit+jitter or boomerang+spiral by default).
...
SPAWN_FIELD: MUST nest configuration inside a "field" object — NEVER put fieldType/radius/strength/durationMs at the action root, and NEVER use "falloff" (engine computes distance falloff automatically):
  { "type": "SPAWN_FIELD", "field": { "fieldType": "MASS_ATTRACTOR"|"RADIAL_IMPULSE"|"VORTEX_TANGENT"|"FRICTION_OVERRIDE", "radius": number, "strength": number, "durationMs": number, "attachToSource"?: boolean, "frictionValue"?: number, "zBase"?: number, "zHeight"?: number, "verticalForce"?: number, "affects"?: "ENEMIES"|"ALLIES"|"CASTER_ONLY"|"ALL", "offset"?: {x,y}, "arcDeg"?: 0-360, "arcFacing"?: "CASTER_FACING"|"CAST_HEADING"|"FIXED", "arcOffsetDeg"?: number } }
  affects: "ENEMIES" (default) | "ALLIES" (summons/allies, not caster) | "CASTER_ONLY" (owner only) | "ALL". CRITICAL: Fields default to ENEMIES only. Personal jump pads MUST use affects: "CASTER_ONLY".
  ...
  arcDeg/arcFacing/arcOffsetDeg: optional wedge. Omitted or arcDeg:360 = full circle. Melee swing recipe: attachToSource:true + arcDeg:90 + arcFacing:"CASTER_FACING" + short durationMs (150-400) + RADIAL_IMPULSE — hits targets in front of the caster only.
...
Cluster Mortar: targetingMode "DIRECTIONAL" (spawns from caster — NEVER GROUND_POINT) + trajectory BALLISTIC_ARC { speed:280-360, lobApex:120-200, maxRange:400-550 } + ON_AIR_APEX -> SPAWN_PROJECTILE fan of child bomblets (BALLISTIC_ARC with bounces:1-3, bounceRestitution, ON_BOUNCE optional SPAWN_FIELD). Alternative: ON_CAST SPAWN_PROJECTILE with same trajectory when no root trajectory field.
